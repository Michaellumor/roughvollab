...
- Runtime figures → `output/` (gitignored if large); figures referenced by
  README → committed under `figures/`. dpi 150.
...
- **D7** Headline figure committed at `figures/layer1b_beta_vs_H.png` so
  the README renders on GitHub even when `output/` is regenerated locally.
