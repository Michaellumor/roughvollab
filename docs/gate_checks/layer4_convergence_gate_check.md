...
**STATUS UPDATE (2026-06-30, D42) — Layer-4 (2a): cross-market replication on live ETH; scale-invariant cleaning + bounds.** Live Deribit ETH (6 maturities / 46 points incl. the 1-year tenor) REPLICATES BTC's two findings: H non-identified (rails to the bound; cond(JᵀJ)=7.95e3, |flat[H]|=0.070, corr(H,ν)=−0.855 — the same H↔ν degeneracy as BTC's −0.875, if anything more degenerate, λ_min 3.6e-3 vs 7.1e-3) and rough-Heston UNDER-produces the crash-fear put tail (signed deep-put residual −0.42 vs BTC −0.35 vol-pts, worst at the long tenor: 1-yr RMSE 3.06 vs 2.43pp). The per-maturity H-sensitivity ||∂IV/∂H|| decays 65.7 (1mo) → 5.9 (1yr) ≈ BTC's 65 → 5.2 — the D41 mechanism (H-signal at the short end, long tenor flat) holds on ETH too. So roughness non-identifiability is a STRUCTURAL cross-market phenomenon, not a BTC idiosyncrasy. **★ Methodological finding:** cross-market calibration needs SCALE-INVARIANT data cleaning AND parameter bounds — a fixed absolute vega/spread/ξ₀ filter is BTC-tuned and silently discards a cheaper, higher-vol market (ETH ~40× lower notional collapsed to 1 maturity / 4 points under VEGA_MIN=5). Fixes (committed, BTC bit-identical by construction): a forward-scaled vega floor (`vega_min × median_forward/FORWARD_REF`) and a data-driven ξ₀ cap (`max(0.25, 1.2×max_ATM_var)`). The ~8h bootstrap was not run (the jacobian + fit settle it — D41 discipline). Figures: `output/eth_smile_fit.png`, `output/eth_h_sensitivity.png`. See ROADMAP **D42**.

**STATUS UPDATE (2026-07-01, D43) — Layer-3 friction sweep: D40's roughness-hedging null is ROBUST across c ∈ [0, 0.05].** The D40 rough-vs-smooth contrast (H=0.10 vs H=0.5, simple features, 8 seeds paired) run at each c ∈ {0, 0.005, 0.01, 0.02, 0.05}: roughness adds no exploitable hedging edge at any friction (c=0.01 reproduces D40 exactly, +0.060/z=1.4). An apparent emergence at c=0.02 (increment +0.362, z=5.6, 7/8) was a RESIDUAL DEEP-CONVERGENCE ARTIFACT — it survived significance, 8/8 seed-consistency, two-budget reproducibility (500ep + 1500ep), an edge-convergence fix (cosine 1500ep monotonised the V-shaped edges but the increment persisted +0.326/z=7.2/8-of-8), and a baseline decomposition (the DELTA_gap was monotonic, ruling out baseline misspecification; the sign-flip lived in the DEEP terms) — and was unmasked only by a 3000ep finer-convergence check: the increment DISSOLVED (+0.033, z=0.7, sign-random) and the DEEP_gap shrank 71% (−0.412→−0.120) WHILE the untrained DELTA_gap control held EXACTLY constant (−0.086→−0.086). **★ Methodological finding:** the edges converging ≠ the finer roughness contrast (a difference-of-differences) converged; statistical significance + seed-consistency + budget-reproducibility did NOT guarantee convergence — deep-hedging budgets must scale with friction, and the untrained baseline is the perfect convergence control. Figure: `output/layer3_friction_convergence.png`. See ROADMAP **D43**.
