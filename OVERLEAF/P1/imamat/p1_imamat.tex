% IMA Journal of Applied Mathematics submission copy of
% ../calibration_paper_revised.tex. Body and thebibliography carried over;
% the class/front-matter block is converted to the OUP IMA authoring template
% (ima-authoring-template.cls v1.01, 2024/05/16, in this folder). The class
% supplies its own layout, hyperref and natbib (namedate = author-year), so
% the article-era geometry/hyperref/amsthm/xcolor loads are dropped. P1 uses
% no theorem-like environments, so none are declared.
...
\title[Identifying roughness from an option surface]{Identifying roughness from an option surface: from a single smile to a live crypto market}

\author{Michael Lumor*\ORCID{0009-0000-0326-3891}
\address{\orgdiv{School of Science, Engineering and Environment}, \orgname{University of Salford}, \orgaddress{\street{New SEE Building}, \postcode{M5 4WT}, \state{Greater Manchester}, \country{United Kingdom}}}}

\authormark{M. Lumor}

\corresp[*]{Corresponding author: \href{mailto:m.lumor@edu.salford.ac.uk}{m.lumor@edu.salford.ac.uk}}

\received{Date}{0}{Year}
\revised{Date}{0}{Year}
\accepted{Date}{0}{Year}

\abstract{Rough stochastic volatility models reproduce the steep short-maturity implied-volatility
skew, with the Hurst exponent $H$ controlling the roughness of the volatility path. We ask
whether $H$ can be \emph{identified} from the object one actually calibrates to---an
implied-volatility surface---via a calibration audit of the rough Heston model from
synthetic known-answer surfaces to two live cryptocurrency options markets. First, a
single maturity cannot identify $H$: the inverse problem pins the variance level, skew and
volatility-of-volatility, but $H$ is the flat eigen-direction, degenerate with the
volatility-of-volatility and corrupted by a tenth of a volatility point of quote noise.
Second, the multi-maturity surface mitigates but does not cure this: the skew term
structure tightens $H$ by roughly an order of magnitude at clean data, yet the degeneracy
persists and returns under realistic noise. Third, on a live Deribit Bitcoin surface $H$
is non-identifiable outright: it rails to its boundary, is the flat direction of the
Jacobian, and two equally good fits land at different parameters. This non-identification
is \emph{intrinsic}---a structural $H$--volatility-of-volatility degeneracy---not a span
artefact: a maturity-adapted Riccati resolution makes the full one-year span computable,
identifiability is unchanged, and the longest tenor is the \emph{least} $H$-informative.
Both findings replicate on Ethereum. This is the option-calibration counterpart of the
observational equivalence documented for roughness estimation from realised variance.
Separately, the model fits the gross crypto smile to within about a volatility point but
systematically under-produces the crash-fear put tail on both markets, even at maximal
roughness. All machinery is validated against the model's characteristic function.}

\keywords{rough volatility; identifiability; inverse problems; model calibration; Fisher information; implied volatility surface.}
...
diffusive models miss~\citep{bfg2016}, and have become a central object of study in quantitative finance
since the work of Gatheral, Jaisson and Rosenbaum~\citeyearpar{gjr2018}. The roughness parameter $H$ is the
...
audited elsewhere~\citep{lumor_identifiability}, with a sobering conclusion: at the roughness
levels real markets exhibit,
...
realised-variance time series---does any better. Calibration is a nonlinear inverse
problem---recovering model parameters from a finite, noisy set of option prices---and the
question is one of parameter identifiability: whether the forward map from parameters to
surface is locally injective in the direction of $H$, diagnosed through the Jacobian's
singular values and the geometry of the fit's flat directions~\citep{kaipio2005}.

The mechanism through which an option surface carries information about $H$ is the term
structure of the at-the-money skew. Fukasawa~\citeyearpar{fukasawa2017} constructs a rough
fractional volatility model dynamically consistent with the power law the SPX skew obeys
in time-to-maturity and derives its short-time implied-volatility expansion; the
short-time skew asymptotics were subsequently sharpened in~\citet{bfghs2019} and~\citet{efgr2019}. On
the calibration side, El Euch, Gatheral and Rosenbaum~\citeyearpar{egr2019} introduced the rough
Heston model as a tractable calibration vehicle and observed that its prices are well
approximated by a classical Heston model with a rescaled volatility-of-volatility, while
Bayer et al.~\citeyearpar{bhmst2019} studied calibration of rough models through learned pricing
maps. None of these works asks whether, given the surface one actually observes, $H$ is
identified at all; that is the question here. It is an open question rather than a solved
one: the tools we use---Jacobian conditioning, flat-direction analysis, noise-perturbed
refits---are standard in inverse problems~\citep{tms2010}, but they have not been turned on this parameter
of this model with market data, and their answer is not the one the calibration literature
assumes.
...
and find $H$ non-identifiable outright: it rails to its bound, is the flat direction
...
equation~\citep{eer2019} and inverting by Gil-Pelaez quadrature~\citep{gilpelaez1951}---and is
fast enough to evaluate the surface the thousands of times an optimiser requires; being exact for
the model, it lets us validate
...
root-mean-square error of about $1.1$ percentage points on the full one-year span
(Figure~\ref{fig:btcfit}). Three of these are
...
Alt text: Six panels, one per maturity, each plotting implied volatility against strike for
the live Deribit Bitcoin surface. The fitted rough Heston curve tracks the market curve
closely at and around the money at every tenor, but sits below it on the deep out-of-the-money
put wing, with the gap widening at the longer maturities.
...
independent diagnostics. The Gauss--Newton curvature at the optimum makes $H$ the flat direction of
the fit---$|{\rm flat}[H]|=0.118$ under the forward-difference Jacobian diagnostic also used in
Section~\ref{sec:live-intrinsic}---within a poorly-conditioned problem ($\mathrm{cond}=1.0\times
...
Alt text: Sensitivity of the implied-volatility smile to the Hurst exponent, plotted against
maturity. The curve is highest at the shortest tenor and falls monotonically towards zero by
one year. A second, near-coincident curve for Ethereum lies on top of the Bitcoin curve.
...
The author thanks Dr Sabine von H\"unerbein (University of Salford) for her guidance as
project advisor.
...
the author, who accepts full responsibility for the contents of this work. All figures
...
\emph{Quant. Finance} \textbf{16}(6), 887--904.

\bibitem[Bayer et al.(2019a)]{bfghs2019} Bayer, C., Friz, P.~K., Gulisashvili, A., Horvath, B.
and Stemper, B. (2019a).
Short-time near-the-money skew in rough fractional volatility models.
\emph{Quant. Finance} \textbf{19}(5), 779--798.

\bibitem[Bayer et al.(2019b)]{bhmst2019} Bayer, C., Horvath, B., Muguruza, A., Stemper, B.
and Tomas, M. (2019b).
...
\emph{Math. Finance} \textbf{29}(1), 3--38.

\bibitem[El Euch et al.(2019a)]{efgr2019} El Euch, O., Fukasawa, M., Gatheral, J.
and Rosenbaum, M. (2019a).
Short-term at-the-money asymptotics under stochastic volatility models.
\emph{SIAM J. Financial Math.} \textbf{10}(2), 491--511.

\bibitem[El Euch et al.(2019b)]{egr2019} El Euch, O., Gatheral, J. and Rosenbaum, M. (2019b).
Roughening Heston. \emph{Risk}, May 2019, 84--89.

\bibitem[Fukasawa(2017)]{fukasawa2017} Fukasawa, M. (2017).
Short-time at-the-money skew and rough fractional volatility.
\emph{Quant. Finance} \textbf{17}(2), 189--198.

\bibitem[Gatheral et al.(2018)]{gjr2018} Gatheral, J., Jaisson, T. and Rosenbaum, M. (2018).
Volatility is rough. \emph{Quant. Finance} \textbf{18}(6), 933--949.

\bibitem[Gil-Pelaez(1951)]{gilpelaez1951} Gil-Pelaez, J. (1951).
Note on the inversion theorem. \emph{Biometrika} \textbf{38}(3--4), 481--482.

\bibitem[Kaipio and Somersalo(2005)]{kaipio2005} Kaipio, J.~P. and
Somersalo, E. (2005). \emph{Statistical and Computational Inverse Problems},
Applied Mathematical Sciences, vol.~160. Springer, New York.

\bibitem[Lumor(2026)]{lumor_identifiability} Lumor, M. (2026).
When is volatility roughness identifiable? A simulation-grounded audit of Hurst
estimation from realised variance, with application to cryptocurrency. Available at
SSRN: \url{https://ssrn.com/abstract=7346318}.

\bibitem[Transtrum et al.(2010)]{tms2010} Transtrum, M.~K., Machta, B.~B.
and Sethna, J.~P. (2010). Why are nonlinear fits to data so challenging?
\emph{Phys. Rev. Lett.} \textbf{104}, 060201.
