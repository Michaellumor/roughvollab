% SIFIN Short Communication submission copy of ../weakorder_paper_clean.tex.
% Body text and thebibliography verbatim; only the class/front-matter block
% converted to SIAM macros (siamonline250211.cls, in this folder — SIAM's
% class for its online-only journals, SIFIN included; siamart251216.cls kept
% alongside for reference). The class loads amsmath/graphicx/hyperref/ntheorem
% itself (NOT amssymb — loaded explicitly below; verified by compile) and
% predefines theorem/lemma/corollary/proposition/definition; remark is
% declared below via \newsiamremark. geometry removed (class layout).
...
\author{Michael Lumor\thanks{School of Science, Engineering and Environment,
University of Salford, UK (m.lumor@edu.salford.ac.uk, ORCID 0009-0000-0326-3891)}}
\headers{Weak order of a direct rough Heston discretisation}{M. Lumor}
...
known to be $H$, the Hurst exponent---so at the empirically relevant $H\approx 0.1$,
pathwise accuracy nearly stalls. Whether \emph{prices} stall too is a question about
the weak order $\alpha$, and there the theory is model-specific: for Gaussian-volatility
models (rough Bergomi and its relatives), recent work establishes weak rates of
$(3H+\tfrac12)\wedge 1$ for left-point discretisation---proven optimal, with a phase
transition at $H=\tfrac16$---and $H+\tfrac12$ for the weighted hybrid scheme. No such
result covers rough Heston, whose square-root Volterra dynamics fall outside these
theorems; known rough-Heston weak-rate evidence concerns Markovian-approximation
schemes, not the direct discretisation. We measure the weak order of the direct
hybrid-type discretisation of rough Heston against a simulation-free
characteristic-function reference, committing before measurement to the prediction
$\alpha>H$. Estimation is made feasible by Romano--Touzi conditioning, which is
bias-preserving by construction and turns the payoff into a smooth functional of the
discretised variance integrals---the object class the theory rates. We find
$\alpha\gg H$ throughout: $\alpha\approx 1$ at $H=0.20$, and a genuine penalty only at
the rough extreme, $\alpha\approx 0.74$ at $H=0.05$ (below $1$ at eight standard
errors)---reproducing, in a model the theorems do not cover, the qualitative phase
structure they prove, with the sub-transition exponent somewhat above the
Gaussian-theory line. The intermediate $H=0.10$ (theory target $0.80$) is not
resolvable at the explicit scheme's $O(n^2)$ cost; a multifactor Markovian lift,
built as the natural remedy, cannot settle it---its noise approximation perturbs the
measured rate itself, a negative result we characterise precisely.
\end{abstract}

\begin{keywords}
rough volatility, rough Heston, weak convergence, discretisation error, Monte Carlo
\end{keywords}
...
$(3H+\tfrac12)\wedge 1$: established by Gassiat~\cite{gassiat2023} for fBm integrands and
cubic test functions, extended and proven \emph{optimal}, with a matching lower bound and a
phase transition at $H=\tfrac16$, by Friz, Salkeld and Wagenhofer~\cite{fsw2025}, and
recovered through path-dependent PDEs by Bonesini, Jacquier and Pannier~\cite{bjp2023}; the
weighted hybrid scheme attains $H+\tfrac12$~\cite{gassiat2023,bfn2022}, and Bayer, Hall and
Tempone~\cite{bht2023} treat option pricing under linear rough volatility. Results in
...
integrated-kernel Euler schemes~\cite{ak2026}, and Bras and Fukasawa treat nonsingular
stochastic Volterra equations~\cite{bf2025}. None of these
...
$L^1$ kernel error~\cite{bb2023} and report second-order weak convergence for a weak
simulation scheme built on it, alongside a numerically observed rate of one for Gatheral's
HQE scheme~\cite{bb2024}---schemes, in both cases, other than the direct discretisation.
...
Breneis~\cite{bb2023,bb2024}. The lift reproduces the strong rate and prices correctly, yet
...
smoothness~\cite{fsw2025,bjp2023}. The measured $\alpha$ is therefore the weak rate of the
...
above the $H=\tfrac16$ phase transition of~\cite{fsw2025}, the theory caps the rate at $1$
and we measure $1.01$: consistent. At $H=0.05$, below the transition, the theory gives
$3H+\tfrac12=0.65$ (left-point, optimal in that setting~\cite{gassiat2023,fsw2025}) and
$H+\tfrac12=0.55$ (weighted hybrid~\cite{gassiat2023,bfn2022}); we measure $0.74$--$0.77$,
...
Heston model of Abi Jaber~\cite{aj2019}: the factors share the single Brownian motion
...
$O(n^2)$ to $O(Nn)$. With the Bayer--Breneis Gauss--Jacobi quadrature of~\cite{bbf2023} the kernel approximation
...
predicts~\cite{bb2023}---the weak error of the \emph{model} approximation is controlled by the
...
Markovian approximation with second-order weak convergence at linear cost~\cite{bb2024}---a
...
settle $H=0.10$, and we have not built one. (The Bayer--Breneis weak scheme~\cite{bb2024}
...
\bibitem{aj2019} Abi Jaber, E. (2019).
...
\bibitem{ak2026} Alfonsi, A. and Kebaier, A. (2026).
...
\bibitem{bf2025} Bras, P. and Fukasawa, M. (2025).
